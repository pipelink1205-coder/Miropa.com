<template>
    <AppLayout title="Publicar anuncio">
        <div class="container-app max-w-3xl py-10">
            <h1 class="text-2xl font-bold text-ink md:text-3xl">Publicar anuncio</h1>
            <p class="mt-1 text-sm text-ink-secondary">Completa los datos de tu artículo.</p>

            <!-- Vertical: Moda vs General -->
            <div v-if="step === 1" class="mt-6 flex gap-2">
                <button
                    type="button"
                    class="rounded-full px-4 py-2 text-sm font-medium transition"
                    :class="vertical === 'fashion' ? 'bg-accent text-white' : 'bg-surface-muted text-ink-secondary'"
                    @click="setVertical('fashion')"
                >
                    Moda
                </button>
                <button
                    type="button"
                    class="rounded-full px-4 py-2 text-sm font-medium transition"
                    :class="vertical === 'general' ? 'bg-accent text-white' : 'bg-surface-muted text-ink-secondary'"
                    @click="setVertical('general')"
                >
                    Otras categorías
                </button>
            </div>
            ...
            <div class="mt-8 rounded-2xl border border-zinc-200 bg-surface-raised p-6">
                ...
                <div v-if="isFashion && step === 5">
                    ...
                    <p class="mt-1 text-sm text-ink-secondary">Confirma antes de publicar.</p>
                    <dl class="mt-4 space-y-2 text-sm">
                        <div class="flex justify-between gap-4 border-b border-zinc-100 py-2">
                            <dt class="text-ink-secondary">Categoría</dt>
                            <dd class="text-right font-medium text-ink">ID {{ form.category_id }}</dd>
                        </div>
                ...
                <div class="mt-8 flex justify-between">
                    ...
                    <div v-else class="flex gap-3">
                        ...
                        <button type="button" class="btn-primary px-6 py-2.5 text-sm" :disabled="form.processing" @click="submit('active')">
